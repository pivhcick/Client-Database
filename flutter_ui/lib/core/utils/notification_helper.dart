...
class NotificationHelper {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      AppLogger.success('Timezone set to: ${tz.local.name}', 'NotificationHelper');
    } catch (e) {
      AppLogger.warning('Could not set timezone by name "$timeZoneName": $e', 'NotificationHelper');

      // Если не получилось, определяем по offset
      final offset = DateTime.now().timeZoneOffset;
      final offsetHours = offset.inHours;
      AppLogger.notification('Device timezone offset: GMT${offsetHours >= 0 ? '+' : ''}$offsetHours', 'NotificationHelper');
      ...
      if (timezoneName != null) {
        try {
          tz.setLocalLocation(tz.getLocation(timezoneName));
          AppLogger.success('Timezone set to: $timezoneName (by offset)', 'NotificationHelper');
        } catch (e2) {
          AppLogger.warning('Could not set $timezoneName, using UTC: $e2', 'NotificationHelper');
        }
      } else {
        AppLogger.warning('Unknown offset GMT$offsetHours, using UTC', 'NotificationHelper');
      }
    }

    AppLogger.success('Timezone initialized: ${tz.local.name}', 'NotificationHelper');
    ...
    if (androidPlugin != null) {
      ...
      AppLogger.success('Android notification channel created: ${androidChannel.id}', 'NotificationHelper');
    }
    ...
    AppLogger.notification('Initializing flutter_local_notifications plugin...', 'NotificationHelper');
    // Initialize
    final initialized = await _notifications.initialize(
      initSettings,
      onDidReceiveNotificationResponse: _onNotificationTapped,
    );
    AppLogger.success('Plugin initialized: $initialized', 'NotificationHelper');
  ...
  Future<bool> requestPermissions() async {
    AppLogger.notification('Requesting notification permissions...', 'NotificationHelper');
    ...
    AppLogger.warning('No platform plugin available', 'NotificationHelper');
    return true;
  }
  ...
  Future<void> scheduleNotification({
    ...
    try {
      ...
      AppLogger.success('zonedSchedule completed successfully', 'NotificationHelper');
    } catch (e) {
      AppLogger.error('Error scheduling notification: $e', 'NotificationHelper');
      rethrow;
    }
  }
  ...
  Future<void> showNotification({
    ...
    try {
      AppLogger.notification('Calling _notifications.show()...', 'NotificationHelper');
      ...
      AppLogger.success('_notifications.show() completed successfully', 'NotificationHelper');
    } catch (e) {
      AppLogger.error('Error showing notification: $e', 'NotificationHelper');
      rethrow;
    }
  }
  ...
  void _onNotificationTapped(NotificationResponse response) {
    AppLogger.notification('Notification tapped: ${response.payload}', 'NotificationHelper');

    // Call the registered callback if available
    if (onNotificationTapped != null) {
      onNotificationTapped!(response.payload);
    } else {
      AppLogger.warning('No onNotificationTapped callback registered', 'NotificationHelper');
    }
  }
}
